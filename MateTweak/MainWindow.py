...
class MateTweak:
    ...
    def enable_autostart(self, filename, content=""):
        ...
        system_autostart = os.path.join(config.sysconfdir, 'xdg', 'autostart', filename) # missed check of: /usr/share/mate/autostart and /usr/share/autostart
    ...
    def disable_autostart(self, filename, comment_add=""):
        ...
        self.mkdir_p(os.path.join(config_dir, 'autostart/'))
        local_autostart = os.path.join(config_dir, 'autostart', filename)
        system_autostart = os.path.join(config.sysconfdir, 'xdg', 'autostart', filename)
    ...
    def replace_windowmanager(self, new_wm):
        ...
        if ('no-composite' in new_wm) and os.path.exists('/etc/xdg/autostart/picom.desktop') and os.path.exists(config_file_picom):
            self.disable_autostart('picom.desktop', 'Reason: parallel running will not disable composite.')
    ...
    def enable_applets(self):
        ...
        self.enable_autostart('mate-volume-control-status-icon.desktop', __SOUND_STATUS_ICON__)
    ...
    def __init__(self, file_path, version, argv):
        ...
        if not self.indicators_available:
            sugg += ["ayatana-indicator-*", "mate-indicator-applet"]
        if not self.mate_menu_available:
            sugg += ["mate-menu"]
        if not self.window_buttons_applet_available:
            sugg += ["mate-window-buttons-applet"]
        if not self.brisk_menu_available:
            sugg += ["mate-applet-brisk-menu"]
        if not self.appmenu_applet_available:
            sugg += ["mate-applet-appmenu"]
        if not self.maximus_available:
            sugg += ["mate-netbook"]
        ...
        if len(sugg) > 0:
            self.builder.get_object("caption_panels_2").set_markup("<small>" + _("Do you want more layout-styles?: Install the suggested packages") + "\n( " + " ".join(sugg) + " )" + "</small>")
            self.builder.get_object("caption_panels_2").set_selectable(True)
        else:
            self.builder.get_object("caption_panels_2").hide()
