...
__APPLET_SOUND_ICON__ = """[Desktop Entry]
Name=Volume Control
Comment=Show desktop volume control
Icon=multimedia-volume-control
Exec=mate-volume-status-icon
Terminal=false
Type=Application
...
class MateTweak:
    ...
    def create_autostart(self, filename, content):
        ...
        if not os.path.exists(os.path.join(config.datadir, 'mate', 'autostart', filename)) and \
           not os.path.exists(os.path.join(config.sysconfdir, 'xdg', 'autostart', filename)):
    ...
    def remove_autostart(self, filename):
        ...
        if os.path.exists(os.path.join(config.sysconfdir, 'xdg', 'autostart',filename)):
    ...
    def enable_autostart(self, filename, autostart_content=""):
        # Enable autostart by remove local disabled .desktop file
        config_dir = GLib.get_user_config_dir()
        local_autostart_file = os.path.join(config_dir, 'autostart', filename)
        system_autostart_file = os.path.join(config.sysconfdir, 'xdg', 'autostart', filename)
        if self.exists_and_enabled_desktopfile(system_autostart_file):
            if self.exists_and_disabled_desktopfile(local_autostart_file):
                    os.remove(local_autostart_file) # User changed state by hand
                    return
        else : # User deleted the system-wide autostart by hand
            if self.exists_and_enabled_desktopfile(local_autostart_file):
                return # nothing to do
            else:
                if autostart_content=="":
                    print("Error: failed to create Autostart (missing content) "+filename)
                    return
                with open(local_autostart_file,'w') as out_file:
                    out_file.write(autostart_content)
    ...
    def disable_autostart(self, filename, comment_add=""):
        ...
        if self.exists_and_disabled_desktopfile(local_autostart_file):
           return # nothing to do
        if self.exists_and_enabled_desktopfile(local_autostart_file):
            os.remove(local_autostart_file)
        if self.exists_and_enabled_desktopfile(system_autostart_file):
            self.copy_system_autostart_to_local_and_disable(system_autostart_file, local_autostart_file, comment_add)

    def copy_system_autostart_to_local_and_disable(self, in_filename, out_filename, comment_add=""):
        # copy a .desktop file and set as disabled (= will not autostart ) and add addational text to comment
        comment_add = "(" + _("This autostart was disabled by Mate-Tweak.") + " "+comment_add + ") "
        input_f = open(in_filename, "r")
        ...
        for line in input_f:
            ...
            output_f.write(line)
        input_f.close()
        output_f.write("Hidden=true");
        output_f.write("# created by MateTweak: This overwrites the system-wide autostart.")
        output_f.close()

    def disabled_desktopfile(self, full_filename):
        # check if .desktop file is disabled (= will not autostart )
        if os.path.exists(full_filename):
            with open(full_filename, 'r') as txt:
                for line in txt:
            ...
            print("file not found to check: " + full_filename)
            return True
        return False

    def enabled_desktopfile(self, full_filename):
        return not self.disabled_desktopfile(full_filename)

    def exists_and_enabled_desktopfile(self, full_filename):
        if(os.path.exists(full_filename)):
            return self.enabled_desktopfile(full_filename)
        return False

    def exists_and_disabled_desktopfile(self, full_filename):
        if(os.path.exists(full_filename)):
            return self.disabled_desktopfile(full_filename)
        return False
    ...
    def check_wm_features(self):
        ...
        self.marco_picom_capable = False
        ...
        if self.find_on_path('marco') and self.find_on_path('picom'):
            if not self.software_rasterizer and self.texture_from_pixmap:
                self.marco_picom_capable = True
    ...
    def replace_windowmanager(self, new_wm):
        ...
        if new_wm == 'marco':
            self.set_bool('org.mate.Marco.general', None, 'compositing-manager', True)
            wm_params = '--composite'
        elif new_wm == 'compiz':
        ...
        # Prevent autostart picom
        # Situations: in case ~/.config/picom.conf ; or in older versions (picom < v12 , before commit: May 23 05:53:43 2024)
        if new_wm == 'marco-no-composite' or new_wm == 'compiz':
            self.disable_autostart('picom.desktop', 'Reason: parallel running compiz has problems') # Remove autostart picom
            self.kill_process('picom') #maybe old autostart progress
    ...
    def enable_dock(self):
        ...
        if not self.process_running(self.dock):
    ...
    def disable_applets(self):
        ...
        self.disable_autostart('mate-volume-control-status-icon.desktop', _('Reason: ayatana-indicator has already a volume controller'))
        ...
        # Remove the outdated mate-volume-control-applet
    ...
    def enable_applets(self):
        ...
        self.enable_autostart('mate-volume-control-status-icon.desktop', __APPLET_SOUND_ICON__)
    ...
    def reload_settings_daemon(self):
    ...
    def replace_panel_layout(self, new_layout, called_from_api=False):
        ...
        # If we're switching to Cupertino move window
        # controls to the left and enable Global Menu.
        # If there is a custom setting, use that first
        layout_name_is_special = self.is_panel_layout_name_special(new_layout, ['eleven','contemporary'])
        ...
        # Change Brisk Menu label-visible state when changing to/from Cupertino and Netbook.
        if self.brisk_menu_available:
            ...
            # If we're switching from Cupertino or Netbook and Brisk Menu label-visible is disabled then enable it.
            elif (self.current_layout.startswith('eleven') or self.current_layout.startswith('netbook')) and not label_visible:
        ...
        if self.brisk_menu_available and self.schema_has_key('com.solus-project.brisk-menu', 'window-type'):
            current_window_type = self.get_enum('com.solus-project.brisk-menu', None, 'window-type')
            if (new_layout.startswith('mutiny') or new_layout.startswith('eleven')) and (current_window_type != 2):
            ...
            elif (self.current_layout.startswith('mutiny') or self.current_layout.startswith('eleven')) and (current_window_type != 1):
        ...
        if os.path.exists(os.path.join(config.datadir, 'mate-panel', 'layouts', new_layout + '.dock')) and self.dock is not None:
            ...
            if not new_layout.startswith('mutiny'):
                self.set_dconf_value('/net/launchpad/plank/docks/dock1/alignment', "'center'")
                self.set_dconf_value('/net/launchpad/plank/docks/dock1/hide-mode', "'window-dodge'")
                ...
                if mate_theme.startswith('Yaru'):
                    ...
                    self.set_dconf_value('/net/launchpad/plank/docks/dock1/theme', "'Default'")
                self.set_dconf_value('/net/launchpad/plank/docks/dock1/zoom-enabled', "true")
            elif new_layout.startswith('mutiny'):
                self.set_dconf_value('/net/launchpad/plank/docks/dock1/alignment', "'fill'")
                self.set_dconf_value('/net/launchpad/plank/docks/dock1/hide-mode', "'none'")
    ...
    def toggle_keyboard_led(self, checkbutton):
        if checkbutton.get_active():
            if not self.keyboard_led_enabled:
                self.enable_keyboard_led()
        else:
    ...
    def additional_tweaks(self, schema, key, value):
        if schema == "org.mate.Marco.general" and key == "button-layout":
            ...
            text = '<b>' + _('Changing the layout will completely replace and reset the current configuration!') + '</b>\n\n' + _('Are you sure you want to replace this layout and potentially lose any customizations you might have made?')
            if not self.confirm_dialog(title, text):
    ...
    def check_keyboard_led_features(self):
        self.keyboard_led_enabled = self.get_bool('org.mate.peripherals-keyboard-xkb.general', None, 'duplicate-leds')
    ...
    def check_panel_features(self):
        ...
        if os.path.exists(os.path.join(config.libdir, 'mate-applets', 'mate-window-applets', 'window-buttons', 'window-buttons-applet')):
    ...
    def delete_panel(self, widget):
        ...
        if 'tweak' in self.current_layout:
            ...
            text = '<b>' + _('Deleting this custom layout will also return your layout to the system default.') + '</b>\n\n' + _('Are you sure you want to permanently delete this custom layout?')
            ...
            if self.confirm_dialog(title, text):
                ...
                if os.path.exists(os.path.join(config.datadir, 'mate-panel', 'layouts', old_layout + '.layout')):
                    return
                Notify.init(_('MATE Tweak'))
    ...
    def save_panel__info_laucher_not_save(self):
        ...
        labelText = _('Your custom or modified launcher in panel will be not saved:')
        path=os.path.join(GLib.get_user_config_dir(), 'mate', 'panel2.d', 'default', 'launchers', '');
        text=""
        counter=0
        #read the .desktop files ; (alternative: use unstable ABI/API from libmate)
        for fileName in glob.glob(path + '*.desktop'):
            ...
            for line in lines:
                ...
                if line[0:4].lower()=="name":
                    if line[0:5].lower()=="name=" : #known bug: "name=" could be outdated ;" name[YOUR_LANGCODE]=" is more up-to-date.
                        pName=line[5:].strip()
                    elif line[0:5].lower()=="name[": #workaround: custom launcher may have only one "name[YOUR_LANGCODE]="
                        counter_nameLoc+=1
                        pName_loc=line[line.find('=')+1:].strip()
            if counter_nameLoc==1:
                pName = pName_loc
        ...
        label2 = Gtk.Label()
        label2.set_markup(_('If these launcher are important for you')+':\n '+_('Do a manual backup of')+' <a href="file://' + path + '">'+_('these files')+'</a>. '+_('To restore: First restore these files, than load the saved panel (other way around: re-login).'))
    ...
    def save_panels(self, widget):
        ...
        if layoutname is not None:
            ...
            if not os.path.exists(os.path.join(config.datadir, 'mate-panel', 'layouts', layoutname + '.layout')):
                return
            Notify.init(_('MATE Tweak'))
    ...
    def __init__(self, file_path, version, argv):
        ...
        self.check_keyboard_led_features()
        ...
        self.builder.get_object("checkbutton_keyboard_led").set_tooltip_text(_("Show keyboard LED indicators in the notifcation area"))
        ...
        # Keyboard LED
        self.builder.get_object("checkbutton_keyboard_led").connect("toggled", self.toggle_keyboard_led)
        self.builder.get_object("checkbutton_keyboard_led").set_active(self.keyboard_led_enabled)
