...
class MateTweak:
    ...
    def enable_autostart(self, filename):
        # enable autostart by remove local .desktop with disabled state (e.g. hidden)
        # This assume that the a working systemwide starter exists
        config_dir = GLib.get_user_config_dir()
        autostart_file = os.path.join(config_dir, 'autostart', filename)
        if os.path.exists(autostart_file):
            if self.is_desktopfile_non_active(autostart_file):
                os.remove(autostart_file)
            else:
                return
        autostart_file_systemwide = os.path.join(config.sysconfdir, 'xdg', 'autostart', filename)
        if os.path.exists(autostart_file_systemwide) and not self.is_desktopfile_non_active(autostart_file_systemwide):
            return
        if not os.path.exists(autostart_file_systemwide) or self.is_desktopfile_non_active(autostart_file_systemwide):
            print("Error: Autostart defect or non working. Please restore this file from origin package: "+autostart_file_systemwide+" (it seems missing or inactive)")
    ...
    def disable_autostart(self, filename, comment_add=""):
        # disable autostart by local .desktop with disabled state (e.g. hidden)
        config_dir = GLib.get_user_config_dir()
        autostart_file = os.path.join(config_dir, 'autostart', filename)
        if os.path.exists(autostart_file):
            if self.is_desktopfile_non_active(autostart_file):
        ...
        autostart_file_systemwide = os.path.join(config.sysconfdir, 'xdg', 'autostart', filename)
        if os.path.exists(autostart_file_systemwide) and not self.is_desktopfile_non_active(autostart_file_systemwide):
            # alternativ simplified version (no Comment added):
              #shutil.copyfile(autostart_file_systemwide,autostart_file)
              #open(autostart_file,'a').write(r"\nHidden=true")
            # copy file and edit: add Comment, add Hidden (=disabled):
            comment="MateTweak disabled the autostart "+comment_add+"; "
            comment_written = False;
            input_f = open(autostart_file_systemwide, "r")
            output_f = open(autostart_file, "w")
            for line in input_f:
            ...
            if not comment_written :
                output_f.write("Comment="+comment)
            output_f.write("Hidden=true");
            output_f.close()

    def is_desktopfile_non_active(self, full_filename):
                ...
                for l in txt:
                    ...
                    if l.find('#')!=-1:
                        if l.find('#')>2:
                            ...
                            continue
                    if l.find('Hidden=true')!=-1 :
                        return True
                    if l.find('NotShowIn')!=-1 and \
                       ( l.find('=MATE')!=-1 or l.find(';MATE')!=-1 ):
                        return True
                    if l.find('OnlyShowIn')!=-1 and len(l)>12:
                        if l.find('=MATE')==-1 and l.find(';MATE')==-1:
                            return True
        else:
    ...
    def disable_applets(self):
        ...
        self.disable_autostart('mate-volume-control-status-icon.desktop', ' ( reason: parallel running ayatana-indicator results in multiple volume controller )')
    ...
    def enable_applets(self):
        ...
        self.enable_autostart('mate-volume-control-status-icon.desktop')
